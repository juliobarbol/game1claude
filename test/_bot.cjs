...
function keyOf(w){
  ...
  return ((p.x/CELL)|0) + ',' + ((p.y/CELL)|0) + ',' +
    ...
    (p.grounded ? 1 : 0) + (p.dashAvail ? 1 : 0) + (p.wall + 1);
}
  ...
  for (let d = 0, dmax = Math.ceil(MAX_FRAMES/K); d < dmax; d++){
    ...
    for (const node of frontier){
      for (let ai = 0; ai < A.length; ai++){
        ...
        if (w.p.win) return { ok:true, path:unroll({ p:node.path, a:ai }), frames:w.timer, expanded };
        const k = keyOf(w);
